    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 page-transition">
      ...
      <div className="text-center mb-10 animate-fade-in">
        ...
        <div className="relative w-24 h-24 mx-auto mb-6">
          ...
          {/* Decorative chips */}
          <div className="absolute -left-2 top-0 w-6 h-6 rounded-full bg-red-500 chip opacity-80 animate-chip-stack" style={{ animationDelay: "100ms" }} />
          <div className="absolute -right-2 top-2 w-5 h-5 rounded-full bg-blue-500 chip opacity-80 animate-chip-stack" style={{ animationDelay: "200ms" }} />
          <div className="absolute left-0 -bottom-1 w-4 h-4 rounded-full bg-green-600 chip opacity-80 animate-chip-stack" style={{ animationDelay: "300ms" }} />
        </div>
        
        <h1 className="text-5xl font-black tracking-tight mb-3">
          <span className="gold-text">Stack</span>
          <span className={isDark ? "text-white" : "text-zinc-800"}>Settle</span>
        </h1>
      ...
      {/* Create Game Form */}
      <form onSubmit={handleCreateGame} className="w-full max-w-md mb-8 animate-fade-in" style={{ animationDelay: "100ms" }}>
        <div className={`flex gap-2 p-2 rounded-2xl transition-all duration-300 ${
          isDark 
          ...
          <button
            ...
            className={`px-6 py-3 font-semibold rounded-xl transition-all duration-300 btn-press ${
              gameName.trim()
      ...
      {currentGames.length > 0 && (
        <div className="w-full max-w-md mb-4 animate-fade-in" style={{ animationDelay: "150ms" }}>
          <button
          ...
          >
            <span className={`text-sm transition-transform duration-200 ${showCurrentGames ? "rotate-90" : ""}`}>▶</span>
            <span className="font-semibold">Live Tables ({currentGames.length})</span>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className={`text-sm ${isDark ? "text-emerald-400" : "text-emerald-600"}`}>
                            {game.players.length} players • {formatCurrency(getGamePot(game))}
                          </p>
                        </div>
                      </div>
                    </div>
      ...
      {pastGames.length > 0 && (
        <div className="w-full max-w-md animate-fade-in" style={{ animationDelay: "200ms" }}>
          ...
          >
            <span className={`text-sm transition-transform duration-200 ${showPastGames ? "rotate-90" : ""}`}>▶</span>
            ...
            <div className="space-y-2 stagger-children">
              {pastGames
                ...
                .map((game) => (
                  ...
                  >
                    <div>
                      <h3 className={`font-medium ${isDark ? "text-zinc-300" : "text-zinc-700"}`}>
                        {game.name}
                      </h3>
                      <p className="text-xs text-zinc-500">
                        {game.players.length} players • {formatCurrency(getGamePot(game))} • {new Date(game.endedAt || game.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <button
      ...
      {/* Footer */}
      <div className="mt-12 text-center animate-fade-in" style={{ animationDelay: "250ms" }}>
